...
struct DailyGoalTextField: View {
    ...
    var body: some View {
        MultilineTextField("What is your goal today?", text: self.$dailyGoal, focus: self.isDailyGoalFocused)
            ...
            .frame(maxHeight: 70)
            .clipped()
            .background(RoundedRectangle(cornerRadius: 4).stroke(self.isDailyGoalFocused.wrappedValue ? Color.lightGray : Color.clear).fill(Color(uiColor: .secondarySystemGroupedBackground)))
